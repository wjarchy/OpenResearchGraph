...
async def test_resume_route_skips_completed_nodes(settings) -> None:
    ...
    # Do not start workers: emulate a saved checkpoint after Architect.
    state["completed_nodes"] = [AgentRole.ARCHITECT.value]
    state["plan"] = {"objective": state["query"], "tasks": [], "assumptions": []}
    runtime.repository.save_checkpoint(state, AgentRole.ARCHITECT.value)

    assert runtime.graph._resume_route(state) == AgentRole.SCOUT.value


async def test_critic_routes_low_quality_research_through_wizard(settings) -> None:
    ...
    assert result is not None
    assert AgentRole.WIZARD.value in result["completed_nodes"]
